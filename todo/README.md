...
- 사용자 변경 제보와 관리자 처리 화면 (`0003_change_reports.sql` 원격 적용 완료)
- 검색 로딩 Skeleton과 오류 복구 화면
- 영문 장소 상세 JSON-LD 구조화 데이터
- 개인정보처리방침과 이용약관 안내
- 외부 수집 요청 지수 백오프 자동 재시도
- 장소 상세의 지도를 최상단 콘텐츠로 배치
- 사용자 화면의 행정·공공데이터 출처 목록과 관련 홍보 문구 제거
- 266개 공개 장소를 이용한 한·영문 상세 페이지 응답 검증
- 결과가 부족했던 세부 검증 조건 단계를 검색 Wizard에서 제거
- 한·영문 헤어 지도 페이지, 지역 필터, 마커 클러스터와 클릭 정보 카드
- 모바일 지도 화면을 가변 높이 지도·선택 카드·가로 장소 목록 구조로 개선
...
3. 지도·상세·검색 결과의 실제 기기 UX 피드백 반영
4. `.env`에 `NEXT_PUBLIC_KAKAO_MAP_JAVASCRIPT_KEY` 설정 후 카카오 JavaScript SDK 실지도 검수
...
4. 가이드 콘텐츠
5. Vercel 배포와 운영 환경변수
6. Analytics와 오류 모니터링
7. 모바일 E2E 테스트
8. Search Console 등록
...
최근 전체 검증 결과(2026-08-07): 웹 12개와 크롤러 16개, 총 28개 테스트 통과. TypeScript 검사와 프로덕션 빌드도 통과했다.
...
- 공개 사용자 화면에는 행정·공공데이터 출처를 노출하지 않는다. 내부 `sources` 데이터는 관리자 검증과 추적을 위해 보존한다.
