...
1. 장소 상세 페이지
2. 모바일 실제 브라우저 레이아웃 검증
3. 예약 링크·전화·공식 홈페이지 이동 처리

### P0 — 관리자 보완

1. 장소 기본 정보 CRUD
2. 서비스·가격·검증 속성·출처 편집 폼
3. 오래된 속성 목록
4. 관리자 작업 이력 조회 화면

### P1 — 데이터 파이프라인

1. 실패 URL 자동 재시도와 실패 원인 저장
...
1. 지역·업종별 랜딩 페이지
2. metadata, canonical, noindex 정책
3. sitemap.xml, robots.txt
4. JSON-LD 구조화 데이터
5. 가이드 콘텐츠
6. Vercel 배포와 운영 환경변수
7. Analytics와 오류 모니터링
8. 개인정보처리방침·이용약관·데이터 출처 표기
9. 모바일 E2E 테스트
10. Search Console 등록
...
- 검색 결과의 `View data`는 장소 상세 UI가 완성되기 전까지 공개 상세 API JSON을 연다.
- 2026-08-07 기준 `/en`과 `/en/search`의 프로덕션 HTML 응답은 검증했지만, 자동 브라우저가 연결되지 않아 데스크톱·모바일 시각 검수는 남아 있다.
