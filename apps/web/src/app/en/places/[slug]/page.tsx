...
export default async function PlaceDetailPage({ params }: Props) {
  ...
  return <main className="place-detail-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replaceAll("<", "\\u003c") }} />
    ...
    <section className="detail-hero">
      ...
      <div className="detail-trust"><span className="verified-dot">Seoul beauty guide</span></div>
    </section>
    <div className="detail-layout">
      <div className="detail-content">
        {place.coordinates && <section className="decision-panel"><span className="eyebrow">LOCATION</span><h2>Know where you’re going</h2><PlaceMap name={place.name} address={place.address ?? `${labels[place.area]}, Seoul`} latitude={place.coordinates.latitude} longitude={place.coordinates.longitude} locale="en" apiKey={process.env.NEXT_PUBLIC_KAKAO_MAP_JAVASCRIPT_KEY ?? ""} /></section>}
        ...
        {(place.menuItems.length > 0 || place.openingHours || place.socialAccounts.length > 0) && <section className="detail-section"><span className="eyebrow">VERIFIED DETAILS</span><h2>Menu, hours & official channels</h2>{place.openingHours && <p><strong>Opening hours:</strong> {place.openingHours.text}</p>}{place.socialAccounts.length > 0 && <div className="booking-links">{place.socialAccounts.map((account) => <a className="button" href={account.url} target="_blank" rel="noreferrer" key={`${account.platform}-${account.url}`}>{account.platform === "instagram" ? `Instagram${account.handle ? ` @${account.handle}` : ""}` : account.platform} ↗</a>)}</div>}{place.menuItems.length > 0 && <div className="service-list">{place.menuItems.map((item) => <article key={item.id}><div><h3>{item.name}</h3></div><div><strong>₩{item.price.toLocaleString("en-US")}</strong><span>Checked {new Date(item.checkedAt).toLocaleDateString("en-US")}</span></div></article>)}</div>}</section>}
