  ...
  return <main className="place-detail-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replaceAll("<", "\\u003c") }} />
    ...
    <div className="detail-layout">
      <div className="detail-content">
        {place.coordinates && <section className="decision-panel"><span className="eyebrow">LOCATION</span><h2>Know where you’re going</h2><div className="place-map"><img src={`/api/v1/map?${new URLSearchParams({ lat: String(place.coordinates.latitude), lng: String(place.coordinates.longitude), width: "900", height: "420" })}`} alt={`Map showing the location of ${place.name}`} /><div className="map-caption"><div><strong>{place.name}</strong><span>{place.address ?? `${labels[place.area]}, Seoul`}</span></div><a className="button secondary" href={kakaoMapLink(place.name, place.coordinates.latitude, place.coordinates.longitude)} target="_blank" rel="noreferrer">Open in Kakao Map ↗</a></div></div></section>}
