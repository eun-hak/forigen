...
const candidates = JSON.parse(await fs.readFile(new URL("kakao-store-image-candidates.json", outputRoot), "utf8"));
...
const existing = await request("place_attributes?attribute_type=eq.hero_image&select=place_id,value_json");
...
const rows = candidates.flatMap((candidate) => {
  ...
  if (current && current.kind !== "kakao_roadview" && current.kind !== "service") return [];
  if (current?.kind === "kakao_roadview") replacements += 1;
  else additions += 1;
  return [{
    ...
    evidence_text: "카카오맵 매장 대표 영역 업주 등록 사진",
...
console.log(JSON.stringify({ researched: candidates.length, withStorePhoto: candidates.filter((item) => item.images?.length).length, imported: rows.length, additions, roadviewReplacements: replacements }, null, 2));
