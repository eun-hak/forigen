...
const candidates = JSON.parse(await fs.readFile(new URL("kakao-remaining-associated-images.json", outputRoot), "utf8"));
...
const rows = candidates.flatMap((candidate) => {
  ...
  return [{
    ...
    value_json: {
      ...
      altKo: `${sourcePlace.nameKo} 카카오맵 연관 방문 사진`,
      altEn: `Associated visitor photo for ${sourcePlace.nameKo}`,
      kind: "kakao_visitor",
      kakaoPlaceId: candidate.id,
    },
    evidence_text: "주소가 일치하는 카카오맵 장소 페이지의 대표·후기 사진",
...
console.log(JSON.stringify({ researched: candidates.length, imported: rows.length, existingImagesPreserved: existing.length }, null, 2));
