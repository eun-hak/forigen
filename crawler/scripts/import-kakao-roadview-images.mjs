...
const candidates = JSON.parse(await fs.readFile(new URL("kakao-roadview-image-candidates.json", outputRoot), "utf8"));
...
const rows = candidates.flatMap((candidate) => {
  ...
  return [{
    ...
    value_json: {
      ...
      altKo: `${sourcePlace.nameKo} 주변 실제 외관 로드뷰`,
      altEn: `Street-view exterior near ${sourcePlace.nameKo}`,
      kind: "kakao_roadview",
      kakaoPlaceId: candidate.id,
    },
    evidence_text: "카카오맵 장소 페이지 로드뷰 사진",
...
console.log(JSON.stringify({ researched: candidates.length, withRoadview: candidates.filter((item) => item.images?.length).length, imported: rows.length, existingImagesPreserved: existing.length }, null, 2));
